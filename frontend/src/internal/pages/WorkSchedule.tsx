  ...
  const processedSlots: ProcessedSlot[] = useMemo(() => {
    return schedules.flatMap((schedule) =>
      schedule.slots.map((slot) => {
        const dateStr = dayjs(slot.startTime).format('YYYY-MM-DD');
        const timeStr = dayjs(slot.startTime).format('HH:mm');
        const endTimeStr = dayjs(slot.endTime).format('HH:mm');
    ...
    <div className='h-[calc(100vh-80px)] flex flex-col mr-5'>
        ...
        >
          ...
          >
            ...
            {weekDates.map((date, index) => {
              const isToday = formatDateAPI(date) === formatDateAPI(new Date());
              return (
                <Tooltip key={index} title={t('workSchedule.tooltipManageDay')}>
                  <div
                ...
                {shift.slots.map((startTime) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span>{startTime}</span>
                        <span className='text-[10px] opacity-60 my-0.5'>-</span>
                        <span>{endTime}</span>
                      </div>

                      {weekDates.map((date) => {
                        const dateStr = formatDateAPI(date);
                        const slotData = getSlotData(dateStr, startTime);
                        ...
                        return (
                          <div
                            ...
                            onClick={() =>
                              handleSlotClick(dateStr, startTime, slotData)
                            }
                          >
                            {slotData && (
                              <div
                              ...
                              >
                                ...
                                {slotData.status === 'pending' && (
                                  ...
                                  >
                                    ...
                                    <span>{t('workSchedule.slotPending')}</span>
                                  </div>
                                )}
                              </div>
                            )}
      ...
      <Modal
        ...
        okButtonProps={{
          ...
          type: 'primary',
          className:
            confirmModal.action === 'OPEN' && confirmModal.scope === 'DAY'
          ...
          {confirmModal.scope === 'DAY' ? (
            <p>
              ...
              <span className='font-bold text-blue-600'>
                {' '}
                {dayjs(confirmModal.date).format('DD/MM/YYYY')}{' '}
              </span>
      ...
      <Modal
        ...
        footer={[
          <Button
            key='close'
            onClick={() => setDetailModal({ isOpen: false, slot: null })}
          >
            {t('common.close', 'Đóng')}
          </Button>
        ]}
      ...
      >
        ...
        detailModal.slot.appointments.length > 0 ? (
              ...
              >
                ...
                >
                  <Tag color='green' className='text-sm px-3 py-1'>
                    {t('workSchedule.statusBooked', 'Đã đặt lịch')}
                  </Tag>
                  <span className='font-semibold text-lg'>
                    {detailModal.slot.startTime} - {detailModal.slot.endTime} |{' '}
                    {dayjs(detailModal.slot.date).format('DD/MM/YYYY')}
                  </span>
                </div>
                ...
                <div className='grid grid-cols-2 gap-4 mt-2'>
                  ...
                  <div className='flex flex-col gap-1'>
                    ...
                    <span className='font-medium'>
                      {getGenderLabel(appt.patientGender, t)}
                    </span>
                  </div>
                  <div className='flex flex-col gap-1'>
                    <span className='text-sm opacity-70'>
                    ...
                    <span className='font-medium'>
                      {appt.patientDob
                        ? dayjs(appt.patientDob).format('DD/MM/YYYY')
                        : '---'}
                    </span>
                  </div>
                  <div className='flex flex-col gap-1'>
                ...
                >
                  <div className='flex flex-col gap-1'>
                    ...
                    >
                      {appt.reason ||
                        t('common.noReason', 'Không có lý do cụ thể')}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className='text-center py-4 text-gray-500'>
            {t('workSchedule.noInfo', 'Không có thông tin chi tiết')}
          </div>
        )}
      </Modal>
    </div>
