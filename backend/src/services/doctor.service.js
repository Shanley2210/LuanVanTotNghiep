...
const getAllDoctorsService = (page, limit, status = null, q = '') => {
    return new Promise(async (resolve, reject) => {
        try {
            const offset = (page - 1) * limit;

            // 1. Điều kiện lọc theo Status (của bảng Doctor)
            let doctorWhere = {};
            if (status) {
                doctorWhere.status = status;
            }
            ...
            const { count, rows } = await db.Doctor.findAndCountAll({
                where: doctorWhere, // WHERE của bảng Doctor
                offset: offset,
                limit: limit,
                distinct: true, // Quan trọng để đếm đúng khi include
                include: [
                    {
                        model: db.User,
                        as: 'user',
                        attributes: ['name', 'email', 'phone'],
                        where: userWhere, // WHERE của bảng User (Search nằm ở đây)
                    },
                    {
                        model: db.Specialty,
                        as: 'specialty',
