...
const getReceptionistsService = (page, limit, status = null, q = '') => {
    return new Promise(async (resolve, reject) => {
        try {
            const offset = (page - 1) * limit;

            // 1. Điều kiện lọc theo Status (của bảng Receptionist)
            let receptionistWhere = {};
            if (status) {
                receptionistWhere.status = status;
            }

            // 2. Điều kiện tìm kiếm (của bảng User - Tên, Email, SĐT)
            let userWhere = {};
            if (q) {
            ...
            const { count, rows } = await db.Receptionist.findAndCountAll({
                where: receptionistWhere,
                offset: offset,
                limit: limit,
                distinct: true, // Đếm chính xác khi dùng include
                include: [
                    {
                        model: db.User,
                        as: 'user',
                        attributes: ['name', 'email', 'phone'],
                        where: userWhere, // Tìm kiếm áp dụng ở đây
                    },
                ],
