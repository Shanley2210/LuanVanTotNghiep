...
const updateDoctorService = (userId, data, imageFile) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (data.ethnicity !== undefined) doctorData.ethnicity = data.ethnicity;
      ...
      if (data.introduce !== undefined) doctorData.introduce = data.introduce;
...
const updatereceptionistService = (userId, data, imageFile) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (data.gender !== undefined) receptionistData.gender = data.gender;
      if (data.ethnicity !== undefined)
        receptionistData.ethnicity = data.ethnicity;
      if (data.address !== undefined) receptionistData.address = data.address;
      if (data.status !== undefined) receptionistData.status = data.status;
      if (imageFile !== undefined)
        ...
        for (const currentShift of shiftList) {
          ...
          while (currentTime < endTime) {
            ...
            currentTime = new Date(
              slotEndTime.getTime() + BREAK_DURATION_MINUTES * 60000,
            );
          }
          ...
          results.push({
            scheduleId: newSchedule.id,
            workDate: normalizedWorkDate.toISOString().split('T')[0],
            shift: currentShift,
            status: 'success',
          });
        }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (existingSchedules.length !== scheduleIds.length) {
        const existingIds = existingSchedules.map((s) => s.id.toString());
...
const setPriceDoctorService = (doctorId, price) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      await db.Doctor.update({ price: price }, { where: { id: doctorId } });
...
const setPriceServiceService = (serviceId, price) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      await db.Service.update({ price: price }, { where: { id: serviceId } });
